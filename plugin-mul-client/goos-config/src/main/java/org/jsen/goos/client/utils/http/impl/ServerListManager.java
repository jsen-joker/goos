 ...
 */
package org.jsen.goos.client.utils.http.impl;

import org.jsen.goos.client.utils.EnvUtil;
import org.jsen.goos.client.utils.IOUtils;
import org.jsen.goos.client.utils.http.impl.EventDispatcher.ServerlistChangeEvent;
import org.jsen.goos.client.utils.http.impl.HttpSimpleClient.HttpResult;
import org.jsen.goos.client.exception.GoosClientException;
import org.jsen.goos.client.utils.ParamUtil;
...
public class ServerListManager {
    ...
    public ServerListManager(String endpoint) throws GoosClientException {
        this(endpoint, null);
    }

    public ServerListManager(String endpoint, String namespace) throws GoosClientException {
        isFixed = false;
        isStarted = false;
        if (StringUtils.isBlank(endpoint)) {
            throw new GoosClientException(GoosClientException.CLIENT_INVALID_PARAM, "endpoint is blank");
        }
        if (StringUtils.isBlank(namespace)) {
            name = endpoint;
    ...
    public ServerListManager(Properties properties) throws GoosClientException {
        isStarted = false;
    ...
    public synchronized void start() throws GoosClientException {
        ...
        if (serverUrls.isEmpty()) {
            LOGGER.error("[init-serverlist] fail to get NACOS-server serverlist! env: {}, url: {}", name,
                addressServerUrl);
            throw new GoosClientException(GoosClientException.SERVER_ERROR,
